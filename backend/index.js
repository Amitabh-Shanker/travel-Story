...
console.log("🔍 Connection string starts with:", connStr ? connStr.substring(0, 30) + "..." : "UNDEFINED");
console.log("🔍 Username:", connStr ? connStr.match(/\/\/([^:]+):/)?.[1] : "NOT FOUND");
...
app.delete("/delete-image", async (req, res) => {
  ...
  try {
    ...
    // Example URL: https://res.cloudinary.com/dsoxsmlfc/image/upload/v123/travel-stories/abc123.jpg
    const urlParts = imageUrl.split('/');
    const filename = urlParts[urlParts.length - 1].split('.')[0]; // abc123
    const folder = urlParts[urlParts.length - 2]; // travel-stories
    ...
    if (result.result === 'ok' || result.result === 'not found') {
      ...
      res.status(400).json({ error: true, message: "Failed to delete image" });
    }
  } catch (error) {
    res.status(500).json({ error: true, message: error.message });
  }
...
// DEV-ONLY: Manual Password Reset Route (REMOVE in production!)
app.post("/reset-password-dev", async (req, res) => {
  const { email, newPassword } = req.body;

  if (!email || !newPassword) {
    return res.status(400).json({ error: true, message: "Email and newPassword are required" });
  }
